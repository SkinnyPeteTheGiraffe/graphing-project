...
import com.notorious.visualization.graphing.algorithm.graph.EdgeWeightedDigraph;
import com.notorious.visualization.graphing.algorithm.graph.EdgeWeightedDirectedCycle;
 ...
 *  @author Kevin Wayne
 */
public class RenderableFloydWarshall {
     ...
     */
    public RenderableFloydWarshall(AdjMatrixEdgeWeightedDigraph G, double[][] coordinates) {
            ...
            for (int v = 0; v < V; v++) {
                ...
                for (int w = 0; w < V; w++) {
                    if (distTo[v][w] > distTo[v][i] + distTo[i][w]) {
                        ...
                        DirectedEdge edge = edgeTo[v][w];
                        Point a = new Point(edge.to(), edge.from());
                        if(!checked.contains(a)) {
                            StdDraw.setPenColor(Color.CYAN);
                            StdDraw.setPenRadius(.002);
                            StdDraw.line(getXCoordinate(edge.from()), getYCoordinate(edge.from()), getXCoordinate(edge.to()), getYCoordinate(edge.to()));
                            StdDraw.setPenColor();
                            StdDraw.filledCircle(getXCoordinate(edge.to()), getYCoordinate(edge.to()), .005);
                            StdDraw.filledCircle(getXCoordinate(edge.from()), getYCoordinate(edge.from()), .005);
                            StdDraw.show(25);
                            checked.add(a);
                        }
                    }
    ...
    public static void main(String[] args) {
        ...
        RenderableFloydWarshall spt = null;//;new RenderableFloydWarshall(G);
